...
<body>
    ...
    <%ResultSet result3;
		...
		String query3 = "SELECT * FROM item i INNER JOIN buyer b ON b.itemID = i.itemID INNER JOIN auction a ON i.itemID = a.auctionID WHERE b.email = '" + email + "'AND valid = 1";
    ...
    <%ResultSet result30;
        ...
        String query30 = "SELECT * FROM item i INNER JOIN buyer b ON b.itemID = i.itemID INNER JOIN auction a ON i.itemID = a.auctionID WHERE b.email = '" + email + "' AND valid = 0";
        System.out.println(query30);
           result30 = stmt.executeQuery(query30);
           %>
    <br>
    <h3>Auctions won as bidder :</h3>
    <table id="tb">
        <tr id="hd1">
            <td>Name</td>
            <td>ItemID</td>
        ...
        <%while (result30.next()) {
              String itemID = result30.getString("itemID");%>
        <tr id="td1">
            <td><%= result30.getString("name") %></td>
            <td><%= result30.getString("itemID") %></td>
            <td>
                <%
                          Statement stmt20 = con.createStatement();
                          String query40 = "SELECT MAX(AMOUNT) AS amount FROM bid WHERE bidID IN ";
                          query40+= " (SELECT bidID FROM bidFor WHERE itemID = " + itemID + ")";
                          ResultSet result40 = stmt20.executeQuery(query40);
                          result40.next();
                          out.write(result40.getString("amount"));
                       %>
